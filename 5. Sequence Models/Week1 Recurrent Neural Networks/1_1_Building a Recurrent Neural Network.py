...
def rnn_cell_backward(da_next, cache):
    ...
    dtanh = da_next * (1 - a_next * a_next)  # 类似于cnn里的dz
...
def rnn_backward(da, caches):
    ...
    for t in reversed(range(T_x)):
        ...
        gradients = rnn_cell_backward(da[:, :, t] + da_prevt, caches[t])  # 每个时间步之后更新梯度,da_prevt是上一个神经元的反向传播，da是自己的y的误差
...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    con = np.vstack((a_prev, xt))
    dWf = np.dot(dft, con.T)
    dWi = np.dot(dit, con.T)
    dWc = np.dot(dcct, con.T)
    dWo = np.dot(dot, con.T)
...
if __name__ == '__main__':
    ...
    a_next, c_next, yt, cache_lstm = lstm_cell_forward(xt, a_prev, c_prev, parameters)
    print("a_next[4] = ", a_next[4])
    ...
    print("cache[1][3] =", cache_lstm[1][3])
    print("len(cache) = ", len(cache_lstm))
    ...
    a_next, yt, cache_bp = rnn_cell_forward(xt, a_prev, parameters)

    da_next = np.random.randn(5, 10)
    gradients = rnn_cell_backward(da_next, cache_bp)
